...
import { calculatePrayers, fetchPrayerTimes, hijriLabel, todayGregorianLabel, type Prayer } from "@/lib/prayer";
...
export default function HomeScreen() {
  ...
  const dark = prayerSettings.theme === "dark";
  ...
  const alerts = prayerSettings.alertsEnabled;
  const sound = prayerSettings.soundEnabled;
  ...
  useEffect(() => {
    ...
    let active = true;
    fetchPrayerTimes(prayerSettings.city.latitude, prayerSettings.city.longitude, new Date(), prayerSettings.method).then(({ prayers: nextPrayers }) => { if (active) setPrayers(nextPrayers); });
    return () => { active = false; };
  }, [prayerSettings.hydrated, prayerSettings.city.id, prayerSettings.method, prayerSettings.locationMode]);
  useEffect(() => { const timer = setInterval(() => { fetchPrayerTimes(coords.latitude, coords.longitude, new Date(), prayerSettings.method).then(({ prayers: nextPrayers }) => setPrayers(nextPrayers)); }, 15 * 60 * 1000); return () => clearInterval(timer); }, [coords.latitude, coords.longitude, prayerSettings.method]);
  ...
  const locate = async () => {
    ...
    try {
      ...
      setCoords(nextCoords); const result = await fetchPrayerTimes(nextCoords.latitude, nextCoords.longitude, new Date(), prayerSettings.method); setPrayers(result.prayers);
  ...
  useEffect(() => {
    ...
    Location.getForegroundPermissionsAsync().then((permission) => {
      if (active && permission.status === "granted") locate();
    }).catch(() => undefined);
    return () => { active = false; };
  }, []);
  ...
  const requestAlerts = async () => {
    ...
    if (permission.status !== "granted") { prayerSettings.setAlertsEnabled(false); Alert.alert(english ? "Notifications disabled" : "التنبيهات متوقفة", english ? "Enable notifications to receive prayer alerts." : "فعّل التنبيهات لتلقي إشعارات الصلاة."); return; }
    prayerSettings.setAlertsEnabled(!alerts);
  };

  return <ScreenContainer edges={["top", "left", "right"]} containerClassName={dark ? "bg-[#10221d]" : "bg-[#f4f7f3]"} className="px-5">
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
      ...
      <View style={styles.hero}><View style={styles.moon}><Text style={styles.moonText}>☾</Text></View><Text style={styles.hijri}>{hijriLabel(now, english)}</Text><Text style={styles.gregorian}>{todayGregorianLabel(english)}</Text><View style={styles.locationRow}><IconSymbol name={"location-on" as any} size={16} color={gold} /><Text style={styles.location}>{locationName}</Text><Pressable onPress={locate} style={styles.refresh}>{loading ? <ActivityIndicator size="small" color={green} /> : <IconSymbol name={"my-location" as any} size={16} color={green} />}</Pressable></View><Text style={styles.locationSource}>{usingCurrentLocation ? (english ? "Prayer times use your current location" : "المواقيت محسوبة حسب موقعك الحالي") : (english ? "Default location: Sana'a" : "الموقع الافتراضي: صنعاء")}</Text></View>
      <View style={[styles.nextCard, dark && styles.darkCard]}><View><Text style={[styles.nextLabel, dark && styles.darkMuted]}>{english ? "NEXT PRAYER" : "الصلاة القادمة"}</Text><Text style={styles.nextName}>{english ? next.en : next.ar}</Text></View><Text style={styles.nextTime}>{next.time}</Text></View>
      <View style={styles.sectionHeader}><Text style={[styles.sectionTitle, dark && styles.darkText]}>{english ? "Prayer times" : "مواقيت الصلاة"}</Text><Pressable onPress={() => router.push("/calendar")}><Text style={styles.link}>{english ? "Calendar" : "التقويم"}</Text></Pressable></View>
      <View style={styles.prayerGrid}>{prayers.map((prayer) => <View key={prayer.key} style={[styles.prayerCard, dark && styles.darkCard, prayer.key === next.key && styles.activePrayer]}><IconSymbol name={prayer.icon as any} size={22} color={prayer.key === next.key ? gold : green} /><Text style={[styles.prayerName, dark && styles.darkMuted]}>{english ? prayer.en : prayer.ar}</Text><Text style={[styles.prayerTime, dark && styles.darkText]}>{prayer.time}</Text>{prayer.key !== "sunrise" && <View style={styles.dot}><IconSymbol name={"notifications-none" as any} size={12} color={prayer.key === next.key ? gold : "#9bb2a9"} /></View>}</View>)}</View>
      <View style={styles.quickRow}><Pressable style={[styles.quickCard, dark && styles.darkQuickCard]} onPress={locate}><IconSymbol name={"location.fill" as any} size={21} color={green} /><Text style={styles.quickText}>{english ? "Update location" : "تحديث الموقع"}</Text></Pressable><Pressable style={[styles.quickCard, dark && styles.darkQuickCard]} onPress={requestAlerts}><IconSymbol name={(alerts ? "bell.fill" : "bell.slash.fill") as any} size={21} color={alerts ? green : "#9aa7a0"} /><Text style={styles.quickText}>{alerts ? (english ? "Alerts on" : "التنبيهات مفعلة") : (english ? "Alerts off" : "التنبيهات متوقفة")}</Text></Pressable></View>
