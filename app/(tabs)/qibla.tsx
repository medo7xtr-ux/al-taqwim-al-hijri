...
export default function QiblaScreen() {
  ...
  return (
    <ScreenContainer edges={["top", "left", "right"]} containerClassName="bg-[#f4f7f3]" className="px-5">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        <View style={styles.headerRow}>
          <View>
            <Text style={styles.kicker}>{english ? "HIJRI • QIBLA" : "هجري • اتجاه القبلة"}</Text>
            <Text style={styles.title}>{english ? "Qibla Compass" : "بوصلة القبلة"}</Text>
          </View>
          <Pressable onPress={() => setEnglish((value) => !value)} style={styles.languageButton}>
        ...
        <View style={styles.readingRow}>
          <View style={styles.reading}><Text style={styles.readingValue}>{Math.round(bearing)}°</Text><Text style={styles.readingLabel}>{english ? "Qibla bearing" : "زاوية القبلة"}</Text></View>
          <View style={styles.divider} />
          <View style={styles.reading}><Text style={styles.readingValue}>{Math.round(distance).toLocaleString()} km</Text><Text style={styles.readingLabel}>{english ? "To Makkah" : "إلى مكة"}</Text></View>
        </View>
        ...
        <Pressable onPress={updateLocation} style={({ pressed }) => [styles.locationButton, pressed && styles.pressed]}>
