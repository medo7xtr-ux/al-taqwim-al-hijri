...
const STORAGE_KEY = "hijri-app-theme-v1";
...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { AsyncStorage.getItem(STORAGE_KEY).then((saved) => { if (saved === "light" || saved === "dark") setColorSchemeState(saved); }).catch(() => undefined); }, []);
  const setColorScheme = useCallback((scheme: ColorScheme) => { setColorSchemeState(scheme); applyScheme(scheme); AsyncStorage.setItem(STORAGE_KEY, scheme).catch(() => undefined); }, [applyScheme]);
